...
For **query** latency at scale (rather than ingest), the `scale_bench` example builds a synthetic index of N docs and times the hot query paths (faceted search, the facet overview, collection page lookups), reporting p50/p95 — see [Scale up](/indice/docs/guides/scale/):

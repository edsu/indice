...
**Response** (abbreviated): `{ "can_annotate": false, "annotations": [ … ] }`, where each note carries its `id`, `author` (a [display name](/docs/guides/annotations/#authorship-and-privacy), never the login identity — this endpoint is public), `created`/`modified`, the target `url` + `timestamp`, the `selector` (the quoted passage, for a passage note), and the note body as both `note_md` (Markdown) and `note_html` (sanitized). `can_annotate` reflects whether the current request is allowed to write.

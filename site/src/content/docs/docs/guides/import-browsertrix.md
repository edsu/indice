...
At the moment an indice server can only be associated with one Browsertrix account. Credentials come from the **environment**, never the command line, so they don't show up in the process list:
...
- **Durable by default.** WACZs are downloaded into `<home>/archive/<item-id>/` (a subfolder per Browsertrix item, so items can't clash on a shared filename), because Browsertrix's presigned URLs expire after ~48h, so a downloaded copy keeps replay working long-term. `--host <URL>` targets a self-hosted Browsertrix (default is `https://app.browsertrix.com`).

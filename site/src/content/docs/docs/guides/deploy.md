---
title: Deploy & run
description: Run indice as a server, using the container image, a one-command Caddy stack with automatic HTTPS, and management over the network.
---
...
- **As a server.** Run the container image behind a TLS-terminating proxy. The included [`compose.yaml`](https://github.com/edsu/indice/blob/main/compose.yaml) does this in one command with [Caddy](https://caddyserver.com), which fetches and renews a Let's Encrypt certificate for you.
...
`/data` is indice's home, holding `archive/`, `collections/`, and `index/`. Mount a volume so it survives restarts. The image runs the read-only server by default.
...
The simplest management needs none of this: `indice serve --manage` on loopback (the [local case](/docs/guides/manage/#local-use)) trusts every request, since it's just you on your machine, with no login. `docker compose up` alone is **read-only**. To open the in-browser management surface to authenticated admins *over the network*, add one of two overlays, both built on the same [forward-auth](/docs/guides/manage/#running-as-a-service-forward-auth) mechanism:

- **Basic auth** (below): a single admin password, with nothing else to run.
- **Single sign-on** ([next section](#single-sign-on-oauth2-proxy)): log in with GitHub, Google, or OIDC via oauth2-proxy, with a real logout. Use this for multiple admins, or where SSO already exists.

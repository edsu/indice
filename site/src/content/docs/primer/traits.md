...
A trait is an interface: a set of methods a type can implement. indice's are
small:
...
safe to move between threads and share between them. Those two are what let the
worker threads in [Concurrency](/primer/concurrency/) call a resolver, and the
compiler checks it.
...
progress update, which is nothing next to the work it reports on.
...
## Default methods, and the null object

A trait method can have a body, and then implementors may skip it. `IndexProgress`
uses this so no caller has to decide whether to report progress:
...
Notice what is *not* derived. `CollectionId` takes `Serialize` from the

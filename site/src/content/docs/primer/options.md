...
absent case. The failure mode is different from null's. You do not get a crash,
you get *ceremony* — code that keeps asking a question whose answer never
mattered.
...
## The question it was asking was not about indexing

That is what makes it the wrong `Option`. Whether a progress bar is on screen is
a fact about the caller's terminal. It is not a fact about indexing a WACZ. The
pipeline was carrying a decision that belonged to the binary, and paying for it
at every call site.
...
absence of one — the **null object** pattern. And since a caller needs something
to point at:
...
whole program and be handed out freely.

The parameter becomes `progress: &dyn IndexProgress`, all twelve `Option`s go,
all twenty-five conditionals go, and the `Option` that remains — whether to draw
a bar — stops in the binary, where the terminal actually is.
...
Before reaching for the pattern, check the trait is actually a *sink* — something
you tell things to, where doing nothing is a legitimate implementation. Progress
reporting is; a `SourceResolver` is not. Resolving a URL has no sensible no-op:
returning nothing is a failure the caller must handle, which is exactly why that
one keeps its `Option`.

So the test is not "can I make this non-optional" but "is there a real
implementation that does nothing?" If yes, write it and delete the branch. If
no, the `Option` is telling you something and should stay.

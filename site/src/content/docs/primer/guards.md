...
Rust has no `finally`. What it has instead is `Drop`: a destructor that runs
when a value goes out of scope, guaranteed, including on an early `return` and
during a panic. Combine that with a value whose whole job is to be held, and you
get a **guard**.
...
The `_index` name is worth a note: the leading underscore says "I am not going
to read this", which silences the unused-variable warning. What it must *not* be
is `_` on its own — a bare `_` drops the value immediately, releasing the lock
on the very next line. That single character is the difference between a held
lock and no lock, and nothing warns you.
...
thread-local. That looks more natural — the guard owns the thing it is
...
backed by nothing, and an ingest ran with no exclusion at all — silently. No
panic, no error, no hang.

Moving the `File` into the thread-local fixes it by making release depend on
**the count reaching zero** rather than on which guard happens to drop first. Out
of order drops become simply correct.

The lesson generalises past locks: when a guard's cleanup has to coordinate with
other guards, the shared thing cannot live in any one of them. And it is a
reminder that `Drop` runs in reverse declaration order *by default* — but
`drop(x)` early, or holding guards in a `Vec` or a struct, breaks that
assumption whenever you were relying on it.
...
**An axum handler cannot hold the lock across an `.await`.** This is the one
worth internalising. In async Rust, a value held across an await point must be
`Send` if the future is going to run on a multi-threaded executor — because the
task can resume on a different thread. So:
...
The compiler refuses. Without `!Send` this would compile happily and hold a
cross-process file lock across an arbitrary await — for as long as an HTTP
request takes, say — which is exactly the kind of thing you find out about in
production. Here the type system just says no.
...
the guard can live there quite happily.

## Guards make invariants where a comment would not

Two more properties of the lock are enforced by ownership rather than by
discipline:
...
- **The lock file is never unlinked.** An `flock` belongs to the inode, so
  ...
  stable across release and re-acquire, because that is exactly the sort of
  "tidy up after yourself" change someone would make later in good faith.
...
- `PhantomData<*const ()>` makes a type `!Send`, which stops it crossing a
  thread *or* an await point — a runtime hazard turned into a compile error.

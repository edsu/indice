...
// Functionally-aligned submodules (leaf-first extraction). `mod.rs` retains the
// public traits, the ingest/reindex/swap core, and re-exports every public item
// so external `index::<symbol>` paths keep resolving unchanged.
mod delete;
mod metadata;
mod optimize;
mod paths;

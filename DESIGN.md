...
- `serve`: opens Tantivy read-only (so `index` can run concurrently), starts Axum. Defaults: `127.0.0.1:8080`.
  - `serve --manage` (opt-in, default **off**) mounts a small **management** write surface on top of the read-only site. Default `serve` mounts none of this and stays strictly read-only. Endpoints + UI:
    ...
    - `GET /manage` (+ `/manage/edit/{id}`) — the browser management page: add-archive form, create/edit-collection form, and the collection list. The homepage also shows an "add your first archive" empty-state CTA under `--manage`.
    - Intended for a trusted, typically **localhost** operator; there is no auth yet, so `--manage` on a non-loopback bind logs a loud startup warning. The networked auth model is still to be decided (`rustyweb-admin-mode-r67p.2`). Browser file-upload (vs. path/URL) and Browsertrix/Archive-It import wizards are tracked follow-ons.

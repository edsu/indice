...
The dominant costs are attacked directly: zstd doc-store compression (`qw5.2`), capping stored `body` text with a snippet fallback (`qw5.3`), and dropping term positions on `headings`/`url_tokens` (`qw5.4`) — all applied on a fresh index or `reindex`. The stored-body cap is operator-configurable (`qw5.8`, see below), so a laptop can keep generous snippets while an institution dials it down. Query-time cost at scale was benchmarked (`qw5.5`, closed): faceted search is sub-linear (bounded by `CANDIDATE_CAP`) and stays under 100 ms at 1M docs; see the `scale_bench` example and the README *Scaling* section. Every such change should be chosen from an `indice stats` before/after, not intuition.

### Distribution & object storage (future, not yet needed)

The scale model says one node is comfortable to ~10M docs (`qw5.7` closed: no sharding needed yet; revisit at ~10 GB index or when facet/phrase latency exceeds budget). When that ceiling is reached, the options — weighed against indice's single-binary, *mutable*-data identity (crawls are deleted/re-collected; finding aids are edited) — are, in preference order:

1. **Local sharding** — several Tantivy indexes (per collection or time bucket) with query-time fan-in. Stays one binary on one machine; buys parallelism and caps per-index build/merge RAM. A stepping stone, not cross-machine.
2. **Object-storage-backed index + read replicas** — the natural "distribute" that keeps the ethos. Tantivy can open an index over a custom `Directory`, so the `index/` could live in an S3-style bucket with a local NVMe read-through cache: one writer publishes an immutable, versioned index; N stateless read-only `serve` replicas mirror shards on demand (cf. the `tantivy-object-store` crate; ~4 s cold / ~20 ms warm in published tests). Swapping `MmapDirectory` for an object-store directory is a contained change (`Index::open(directory)`), not a rewrite. Deletes stay workable via versioned rebuilds (readers pin a version).
3. **Quickwit** — a distributed search engine built *on* Tantivy (splits on object storage, petabyte scale). Deliberately excluded from the core: it's append-only/immutable-split (deletes are expensive and delayed — a poor fit for indice's routine deletes/re-collection) and a multi-component cluster, the opposite of one self-contained binary. An escape hatch only if indice ever pivots toward log-scale, append-only workloads.
4. **Federation** (`rustyweb-federation-shwl`) — a different axis: independent indice instances that cross-search/cross-link, distributing by *node* (e.g. per institution) rather than sharding one logical index.

Related idea worth capturing (tracked as `rustyweb-object-storage-home`): the **`archive/` directory itself** (the WACZ store) could optionally be object-storage-backed too. WACZs are already served over HTTP range reads at `GET /files/{id}`, so backing that with an S3-style bucket (serve_file range-proxying or redirecting to presigned URLs) is a natural fit and pairs with (2) — an entirely object-storage-resident home (`archive/` + `index/`) for a cloud deployment.
...
gated write); authoring is authenticated (any curator), and edit/delete is own-only, with
admins able to moderate anyone's note. A note records a normalized **subject id** (compared,
never shown) separately from a **display name** (shown, never compared) — annotations are
world-readable and the JSONL is committed, so publishing a raw login identity would put
contributors' addresses into git history. Author matching canonicalizes the *stored* key, so
notes written before identities were normalized stay editable by their authors without a
migration.

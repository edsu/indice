...
- `serve`: opens Tantivy read-only (so `index` can run concurrently), starts Axum. Defaults: `127.0.0.1:8080`.
  - `serve --manage` (opt-in, default **off**) mounts a small **management** write surface on top of the read-only site — currently `POST /api/archives`, which reuses the exact `index::index_location` path the CLI uses to add a crawl to a collection. It runs on a blocking thread (returns a job id immediately), streams progress over Server-Sent Events at `GET /api/archives/{id}/events`, and on success **hot-reloads** the read-only searcher (the server holds it behind an `RwLock<Arc<SearchIndex>>` and swaps in a freshly-opened index) so new results appear without a restart. The server still never holds Tantivy's write lock itself — the ingest opens its own short-lived writer, exactly as the CLI does. Default `serve` mounts none of this and stays strictly read-only. Intended for a trusted, typically localhost, operator; a networked auth model is still to be decided (`rustyweb-admin-mode-r67p.2`).

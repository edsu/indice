...
- `serve`: opens Tantivy read-only (so `index` can run concurrently), starts Axum. Defaults: `127.0.0.1:8080`.
  - `serve --manage` (opt-in, default **off**) mounts a small **management** write surface on top of the read-only site. Default `serve` mounts none of this and stays strictly read-only. Endpoints + UI:
    ...
    - `GET /manage` (+ `/manage/edit/{id}`) — the browser management page: add-archive form (upload **or** path/URL), create/edit-collection form, and the collection list. The homepage shows an "add your first archive" empty-state CTA, and each collection page an "Edit collection" link, under `--manage`.
    - **Auth.** Two modes:

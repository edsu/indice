...
`.concurrency()`, `.resolver()`, `.progress()`, `.actor()`). `index_path` stays
a free function for the common "just index this file" case.
...
`.actor()` is that claim cashed in: custody became a field, a setter and one
expression in `record::upsert`, and the ~55 lines of snapshot machinery at the
server were deleted. `archiveit::import_crawls` takes the configured `Ingest`
